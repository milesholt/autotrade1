...
//Require strategy
const strategy = require('./strategies/breakoutStrategy-backtest.js');
//Require analytics
...
const date2 = moment(date1).subtract(1, 'months').format('YYYY-MM-DD');
...
async function exec(){
  ...
  //If we have already stored the data from API, we can now call it from the path
...
async function testStrategy(i,data){
  ...
  await strategy.actions.exec(epic, dataset).then(async r => {
    strategydata.push(r);
