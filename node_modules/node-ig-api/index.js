...
function _request(method, path, payload, extraHeaders) {
	...
	demo = process.env.IG_DEMO==='TRUE'?true:false;
	let headers = {
			...
			res.on('end', () => {
				try {
					resolve({
						status,
						headers,
						body: body.length === 0 ? {} : tryJSON(body) // cannot parse an empty body, also check resopnse is JSON not HTML
					});
				} catch (e) {
					reject({
						status,
...
function refreshToken() {
	return new Promise((res, rej) => {
		...
		return _request('POST', '/session/refresh-token', false, extraHeaders)
			.then(r => {
				if (r.status !== 204) {
					rej(r);
				} else {
...
function subscribeToLightstreamer(subscriptionMode, items, fields, maxFreq, streamLogDir, epic) {
	...
	//First check if there is already a subscription with that epic
	if(isSubscribed(epic) !== false){
		console.log('Epic is already subscribed.');
		return false;
	}
...
function getActiveSubscriptions(){
	return lsClient.getSubscriptions();
}

function isSubscribed(epic){
	if(subscriptions[epic]){
		return subscriptions[epic].isSubscribed();
	} else {
		console.log('Tried to check for if subscribed, but could not find on subscriptions, epic: ' + epic);
		lsIsError = true;
		return false;
	}
...
function isActive(epic){
	return subscriptions[epic].isActive();
}
