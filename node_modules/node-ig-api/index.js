...
function switchDefaultAcct(accountId) {
	return new Promise((res, rej) => {
		let tokens = require(tokensDir);
		let payload = {
			'accountId': accountId,
...
// Return one position
function getPosition(dealId) {
	return new Promise((res, rej) => {
		get('/positions/'+dealId, 2)
			.then(r => {
				if (r.status !== 200) {
					rej(r);
				} else {
...
function showOpenPositions(version = 2) {
	return new Promise((res, rej) => {
...
function closePosition(dealId) {
	return new Promise((res, rej) => {
		//let temp1 = []; //original, but this should be object?
		let temp1 = {};
		let response = {};
		get('/positions', 2)
			.then(r => {
				if (r.status !== 200) {
					...
					if(temp2.length){
						...
						rej('No positions found. Position might have already been closed by server.');
					}
				}
			})
			.then(r => {
...
function histPrc(epic, resolution, from, to) {
	...
	return new Promise((res, rej) => {
		let url = '/prices/' + epic + '/' + resolution + '/' + from + '/' + to;
		get(url,2)
			.then(r => {
				if (r.status !== 200) {
					rej(r);
	...
	requirejs(['LightstreamerClient'], function(LightstreamerClient) {
		...
		lsClient.addListener({
			...
			onStatusChange: status => {
				...
				if(status == 'CONNECTED:WS-STREAMING') lsIsConnected = true;
			},
...
function isConnectedToLightStreamer(){
	//console.log(Object.getOwnPropertyNames(lsClient).length );
	return lsIsConnected;
}

// Subscribe to lightstreamer
function subscribeToLightstreamer(subscriptionMode, items, fields, maxFreq, streamLogDir, epic) {
	/**
	...
	requirejs(['Subscription'], function(Subscription) {
		...
		subscription.addListener({
			...
			onItemUpdate: updateInfo => {
				...
				var writerStream = fs.createWriteStream(streamLogDir);
...
// Unsubscribe to lightstreamer
function unsubscribeToLightstreamer(epic) {
	lsClient.unsubscribe(subscriptions[epic]);

}

// Unsubscribe to lightstreamer
function disconnectToLightstreamer() {
	lsClient.disconnect();
}
...
module.exports = {
	...
	switchDefaultAcct: switchDefaultAcct,
	acctInfo: acctInfo,
	...
	disconnectToLightstreamer: disconnectToLightstreamer,
	subscribeToLightstreamer: subscribeToLightstreamer,
	unsubscribeToLightstreamer: unsubscribeToLightstreamer,
