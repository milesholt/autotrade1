                                ...
                                console.log(markets[x.marketId].trailingStop);
                                 ...
                                 if ((dir === "BUY" && currentPrice > profitThreshold) || (dir === "SELL" && currentPrice < profitThreshold)) {
                                   ...
                                   if(index == 1){
                                    ...
                                    //First setup trailingstop when 60% profit reached
                                    //Trailing distance needs to be 60% of points differnece between currentPrice and stopLevel
                                    let difference = Math.abs(p.stopLevel - currentPrice); // Absolute difference
                                    let trailingStopDistance = difference * 0.60;
...
//Only proceed if trailingstop already tried and not null
if (isTrailingStopDefined) {
  //only if trailing stop rejected
  if(markets[x.marketId].trailingStop == false){
            ...
            if (shouldAdjust) {
                ...
                await api.editPosition(x.dealId, adjustData).then(r => {
                    if (r.dealStatus == 'ACCEPTED') {
                        ...
                        if(adjust50) markets[x.marketId].adjustedStop50 = false;
                        if(adjust80) markets[x.marketId].adjustedStop80 = false;
                    }
                }).catch(e => console.log(e));
            } else {
                if(index == 1){
                  ...
                  shouldAdjust = false;
                                   ...
                                   //only if trailing stop not tried
