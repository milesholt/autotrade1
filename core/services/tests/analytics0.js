...
actions.drawChart = async function(priceData, lineData, analysis, rangeData){
  ...
  let rangeLimitPerc = 0.45;
  let tradeLimitPerc = 0.3;
  let lineDistanceLimitPerc = 0.05;
  let limitDistancePerc = 0.08;
  //end
  ...
  //This takes place in analysisHandler

  let times = [], customdata = [], closes = [], opens = [], highs = [], lows = [], range = [], rangeAnalysis = [];
  ...
  let lowestPrice = rangeAnalysis[0];
  let highestPrice = rangeAnalysis[rangeAnalysis.length-1];
  let priceDiff = lib.toNumber(highestPrice - lowestPrice);
  let rangelimit = (priceDiff * rangeLimitPerc);
  ...
  //Calculated in checkHandler
  let momentumLimit = 0;
  let tradelimit = 0;
  let linedistancelimit = 0;
