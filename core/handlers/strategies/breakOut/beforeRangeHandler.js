...
actions.determineBeforeRangeTrend = async function(){
  ...
  for(let i =0; i<=11; i++){
    if(trend == 'bullish' && (lib.toNumber(pricedata3.support[i].close) <= lib.toNumber(lineData.support)) ) isBeforeRangeTrendNotBroken = true;
    if(trend == 'bearish' && (lib.toNumber(pricedata3.support[i].close) >= lib.toNumber(lineData.resistance)) ) isBeforeRangeTrendNotBroken = true;
  }
