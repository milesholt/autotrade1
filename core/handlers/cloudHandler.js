...
actions.syncFile = async function (cloudDataDir, localFile, mapProperty) {
  ...
  tmp_cloud.forEach((cfield, i) => {
    tmp_local.forEach((lfield) => {
      if (cfield[mapProperty] == lfield[mapProperty])
        tmp_cloud[i] = Object.assign({}, lfield, cfield);
    });
  });
  ...
  tmp_cloud.forEach((tc, i) => {
    if (lib.isDefined(tc, "deal") && lib.isDefined(cloudFile[i], "deal"))
      tc.deal = cloudFile[i].deal;
    if (lib.isDefined(tc, "data") && lib.isDefined(cloudFile[i], "data"))
      tc.data = cloudFile[i].data;
    if (lib.isDefined(tc, "tradedBefore")) {
      if (cloudFile[i]) {
        if (typeof cloudFile[i].tradedBefore == "undefined")
          cloudFile[i].tradedBefore = false;
