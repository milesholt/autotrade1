...
actions.calcResistSupport = async function(pricedata,type){
  ...
  let rangediff_m1 = parseFloat(rangediff * 0.1).toFixed(2);  //
  let rangediff_m2 = parseFloat(rangediff * 0.3).toFixed(2); //20% of range

  //TO LOOK INTO - Perhaps percentage of margin2 needs to be determined by difference between highest and lowest in range data

//  console.log('margin1 - 8% of range difference: ' + rangediff_m1);
  //console.log('margin2 - 20% of range difference: ' + rangediff_m2);


  let margin1 = rangediff_m1;  // goes through all prices and uses their midrange value to capture range data
  let margin2 = rangediff_m2; // goes through range data and uses margin to determine range area, and support / resistance lines
  ...
  */
  ...
  const margins = [0.1,0.15,0.2,0.25,0.3];
  let rangeOptions = [];
  ...
  console.log('primary margin:');
  console.log(primary);



  //new method using margin2
  // let differences = [];
  // rangeData[type].prices.forEach((price,idx) => {
  //   price = parseFloat(price);
  //   let match = false
  //   //get difference of previous bar
  //   if(idx > 0){
  //     let p1 = rangeData[type].prices[idx-1];
  //     let p2 = price;
  //     let d = Math.abs(p1 - p2);
  //     differences.push(parseFloat(d.toFixed(2)));
  //   }
  // });
  ...
  midrangeprice = (highestareaprice + lowestareaprice) / 2;
  lineData.midrange = midrangeprice;
