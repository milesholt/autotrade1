...
actions.determineConfirmations = async function(){
  ...
  waves.forEach((wave,idx) =>{
    if(idx > 0 && idx < waves.length-1){
      ...
      let nextTimeDiff = Math.abs(moment.utc(wave.time).local().diff(moment.utc(nextTime).local(), "hours"));
      let prevTimeDiff = Math.abs(moment.utc(wave.time).local().diff(moment.utc(prevTime).local(), "hours"));
