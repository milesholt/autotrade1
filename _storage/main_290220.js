...
async function exec(){
  ...
  if(prices.length === 0){
    ...
    await api.histPrc(epic, resolution, from, to).then(r => {
      ...
      console.log(e);
      pricedatacount = r.prices.length;
...
if(noError){
  ...
  if(lineDistance > 20 && lineDistance < 200) check0 = true;
  ...
  await analytics.actions.drawChart(pricedata2.support, wickdata, linedata, analysis);
